...
public:
	TextLabelComponent(std::string text, std::string fontId, const SDL_Color &color) {
		_text = std::move(text);
	...
	void SetLabelText(std::string text) {
		_text = std::move(text);
		SDL_Surface *surface = TTF_RenderText_Blended(Game::GlobalAssetManager->GetFont(_fontId),
		                                              _text.c_str(), _color);
